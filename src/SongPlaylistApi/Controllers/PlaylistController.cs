...
public class PlaylistController(IPlaylistService service) : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create(PlaylistRequest request, CancellationToken ct)
    {
    ...
    public async Task<IActionResult> RemoveSong(int playlistId, int songId, CancellationToken ct) =>
        await service.RemoveSongAsync(playlistId, songId, ct) ? NoContent() : NotFound();
}
