...
public class SongService(ISongRepository repo) : ISongService
{
    ...
    public Task<Song> CreateAsync(SongRequest r, CancellationToken ct = default) => repo.CreateAsync(new Song { Name = r.Name, Artist = r.Artist, Album = r.Album, Genre = r.Genre, Duration = r.Duration, ReleaseDate = r.ReleaseDate }, ct);
    public Task<bool> UpdateAsync(int id, SongRequest r, CancellationToken ct = default) => repo.UpdateAsync(new Song { Id = id, Name = r.Name, Artist = r.Artist, Album = r.Album, Genre = r.Genre, Duration = r.Duration, ReleaseDate = r.ReleaseDate }, ct);
